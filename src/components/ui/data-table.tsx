...
export function DataTable<TData, TValue>({
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Table */}
      <div className="rounded-xl border overflow-hidden border-[var(--color-border)] bg-[var(--color-bg-card)]">
        <Table>
          <TableHeader>
            {table.getHeaderGroups().map(hg => (
              <TableRow key={hg.id} className="border-[var(--color-border)] hover:bg-transparent">
                        ...
                        {header.column.getCanSort() && (
                          <span className="opacity-50">
                            {header.column.getIsSorted() === "asc"
                              ...
                              : <span className="flex flex-col"><ChevronUpIcon className="w-2.5 h-2.5 -mb-0.5" /><ChevronDownIcon className="w-2.5 h-2.5" /></span>
                            }
                          </span>
                        )}
                      </div>
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              table.getRowModel().rows.map(row => (
                <TableRow key={row.id} className="border-[var(--color-border)] hover:bg-[var(--color-bg-card-hover)] transition-colors">
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-20 text-center text-[var(--color-text-muted)] text-sm">
                  No results.
                </TableCell>
              </TableRow>
            )}
