...
export async function POST(req: NextRequest) {
  ...
  const { divisionId, groupId, courseId, facultyId, date, slotNumber, roomId, activityType } = body;

  if (!courseId || !date || !slotNumber || (!divisionId && !groupId)) {
    return NextResponse.json(
      { error: "courseId, date, slotNumber, and either divisionId or groupId required" },
      { status: 400 }
    );
  }
  ...
  if (!slotDef) {
    return NextResponse.json({ error: `Invalid slot number ${slotNumber}. Must be 1-8.` }, { status: 400 });
  }
  ...
  if (facultyId) {
    ...
    if (conflictDraft && conflictDraft.courseId !== courseId) {
      return NextResponse.json({ error: "Faculty already has a draft slot at this time." }, { status: 409 });
    }
    const conflictLive = await prisma.timetable.findFirst({
      where: { facultyId, date, slotNumber: parseInt(slotNumber) },
    });
    if (conflictLive && conflictLive.courseId !== courseId) {
      return NextResponse.json({ error: "Faculty is already scheduled in the live timetable for this slot." }, { status: 409 });
    }
  }
  ...
  try {
    ...
    const upsertWhere = divisionId
      ? { divisionId_date_slotNumber: { divisionId, date, slotNumber: parseInt(slotNumber) } }
      : { groupId_date_slotNumber: { groupId: groupId!, date, slotNumber: parseInt(slotNumber) } };
    ...
    if (error.code === "P2002") {
      return NextResponse.json({ error: "Slot already occupied for this division/group on this date/slot" }, { status: 409 });
    }
    throw err;
  }
}
