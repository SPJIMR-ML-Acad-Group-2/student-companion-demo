...
export async function POST(req: NextRequest) {
  ...
  const { weekOf, visibility = "confirmed" } = body as { weekOf: string; visibility?: string };

  if (!weekOf) {
    return NextResponse.json({ error: "weekOf required (YYYY-MM-DD)" }, { status: 400 });
  }

  // Compute Mon-Sat of the given week
  ...
  if (drafts.length === 0) {
    return NextResponse.json({ published: 0, message: "No draft entries for this week." });
  }
  ...
  for (const draft of drafts) {
    ...
    if (draft.activityType === "session") {
      const maxEntry = await prisma.timetable.findFirst({
        where: {
          ...
          ...(draft.divisionId ? { divisionId: draft.divisionId } : { groupId: draft.groupId }),
        },
    ...
    try {
      ...
      const upsertWhere = draft.divisionId
        ? { divisionId_date_slotNumber: { divisionId: draft.divisionId, date: draft.date, slotNumber: draft.slotNumber } }
        : { groupId_date_slotNumber: { groupId: draft.groupId!, date: draft.date, slotNumber: draft.slotNumber } };
  ...
  await prisma.draftTimetable.updateMany({
    where: { date: { in: weekDates }, isPublished: false },
    data: { isPublished: true },
  });

  return NextResponse.json({ published, skipped, weekDates });
}
