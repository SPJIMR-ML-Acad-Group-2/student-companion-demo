...
export async function GET(req: NextRequest) {
  ...
  // Build working date list (Mon–Sat, no Sundays)
  const allDates: string[] = [];
  for (let d = new Date(termStart + "T00:00:00"); d <= new Date(termEnd + "T00:00:00"); d.setDate(d.getDate() + 1)) {
    if (d.getDay() !== 0) allDates.push(localDateStr(d));
  }

  // Load core divisions (with students — to find their group memberships)
  ...
  for (const coreDiv of coreDivisions) {
    ...
    // ── Gather group IDs for students in this core division ──────────────
    const studentIds = coreDiv.students.map((s) => s.id);

    const studentGroupRows = studentIds.length
    ...
    const groupIds = [...new Set(studentGroupRows.map((sg) => sg.groupId))];
    ...
    const groupTT = groupIds.length
      ? await prisma.timetable.findMany({
          where:   { groupId: { in: groupIds }, date: { in: allDates } },
          include: {
            course: true,
            faculty: true,
            room: true,
    ...
    // Lookups: "date-slot" → entry
    type CoreEntry = typeof coreTT[number];
    type GroupEntry = typeof groupTT[number];

    const coreMap = new Map<string, CoreEntry>(coreTT.map((e) => [`${e.date}-${e.slotNumber}`, e]));
    const groupMap = new Map<string, GroupEntry>(groupTT.map((e) => [`${e.date}-${e.slotNumber}`, e]));

    if (!coreDiv.erpClassCode) warnings.push(`Division "${coreDiv.name}" missing erpClassCode`);

    // ── Generate ALL date × ERP-period rows ───────────────────────────────
    let rowIdx = 4;           // 0-based → starts at row 5 in Excel
    let ttdid  = 692385;      // Auto-incrementing ERP system ID (demo values)
    ...
    for (const date of allDates) {
      ...
      for (const ep of ERP_PERIODS) {
        ...
        sc(ws, rowIdx, COL.B, 2);
        sc(ws, rowIdx, COL.C, 181);
        sc(ws, rowIdx, COL.D, 682);
        sc(ws, rowIdx, COL.E, 8811);
        sc(ws, rowIdx, COL.F, ttdid++);
        sc(ws, rowIdx, COL.G, 3165);
        sc(ws, rowIdx, COL.H, "SPJMUM");
        sc(ws, rowIdx, COL.I, "BATCH0323");
        sc(ws, rowIdx, COL.J, "SES0034");
        sc(ws, rowIdx, COL.K, coreDiv.name);
        sc(ws, rowIdx, COL.L, coreDiv.erpClassCode ?? "");
        sc(ws, rowIdx, COL.M, weekday);
        sc(ws, rowIdx, COL.N, dateErp);
        sc(ws, rowIdx, COL.O, ep.period);
        sc(ws, rowIdx, COL.P, ep.timing);
        sc(ws, rowIdx, COL.R, "");   // empty string (matches template default)
        sc(ws, rowIdx, COL.AR, "N");
        sc(ws, rowIdx, COL.AS, "N");
        sc(ws, rowIdx, COL.AT, "N");

        // ── Fill in class-specific columns if a slot is scheduled ───────
        if (ep.appSlot !== null) {
          const key        = `${date}-${ep.appSlot}`;
          const coreEntry  = coreMap.get(key);
          const groupEntry = groupMap.get(key);
          const entry      = coreEntry ?? groupEntry;

          if (entry) {
            const isGroup = !coreEntry && !!groupEntry;
            ...
            if (entry.activityType === "session" && entry.sessionNumber != null)
              sc(ws, rowIdx, COL.Q, entry.sessionNumber);

            // R: subject code (Mandatory)
            sc(ws, rowIdx, COL.R, entry.course.erpSubjectCode ?? entry.course.code);

            // S: activity code (Mandatory)
            sc(ws, rowIdx, COL.S, ACTIVITY_CODE_MAP[entry.activityType] ?? ACTIVITY_CODE_MAP.session);

            // T: Faculty Code1, U: Is Assistant1
            if (entry.faculty?.erpCode) {
              sc(ws, rowIdx, COL.T, entry.faculty.erpCode);
              sc(ws, rowIdx, COL.U, "N");
            ...
            if (entry.room?.erpCode) {
              sc(ws, rowIdx, COL.AN, entry.room.erpCode);
            } else if (entry.roomId) {
              warnings.push(`Room ID ${entry.roomId} missing erpCode`);
            }

            // AP: Group Code (group-based classes only)
            if (isGroup && groupEntry) {
              const grp = groupEntry.group;
              if (grp?.erpGroupCode) {
                sc(ws, rowIdx, COL.AP, grp.erpGroupCode);
              } else {
                warnings.push(`Group "${grp?.name ?? "unknown"}" missing erpGroupCode`);
              }
            }
          }
        }
  ...
  if (uniqWarnings.length > 0) {
    ...
    const warnWs = XLSX.utils.aoa_to_sheet([["Warning"], ...uniqWarnings.map((w) => [w])]);
    XLSX.utils.book_append_sheet(warnWb, warnWs, "Warnings");
    zip.file("_WARNINGS.xlsx", XLSX.write(warnWb, { type: "buffer", bookType: "xlsx" }));
  }

  const zipBuf = await zip.generateAsync({ type: "nodebuffer" });
  const ab = zipBuf.buffer.slice(zipBuf.byteOffset, zipBuf.byteOffset + zipBuf.byteLength) as ArrayBuffer;
