...
function ProgressRing({ percentage, size = 72 }: { percentage: number; size?: number }) {
  ...
  const color = percentage >= 85 ? "#22c55e" : percentage >= 75 ? "#f58220" : "#ef4444";
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <span className="absolute inset-0 flex items-center justify-center font-bold" style={{ color, fontSize: size * 0.22 }}>
        {percentage}%
      </span>
    </div>
  );
...
export default function StudentDashboard() {
  ...
  const statCards = [
    { label: "Overall",  value: `${overallPct}%`, color: overallPct >= 85 ? "#22c55e" : overallPct >= 75 ? "#f58220" : "#ef4444", sub: `${totalPresent + totalLate}/${totalSessions}` },
        ...
        {statCards.map(({ label, value, color, sub }) => (
          <Card key={label} className="bg-[var(--color-bg-card)] border-[var(--color-border)]">
            <CardContent className="p-4">
              <p className="text-xs uppercase tracking-widest mb-2 text-[var(--color-text-muted)]">{label}</p>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-[var(--color-border)]">
                  ...
                  <TableHead className="text-[var(--color-text-muted)]">L1 (1-level ↓)</TableHead>
                  <TableHead className="text-[var(--color-text-muted)]">L2 (2-level ↓)</TableHead>
                  <TableHead className="text-[var(--color-text-muted)]">L3 (F grade)</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {[
                  ...
                  <TableRow key={credits} className="border-[var(--color-border)]">
                    <TableCell className="text-[var(--color-text-primary)] font-medium">{credits}</TableCell>
                    <TableCell className="text-[var(--color-text-secondary)]">{sessions}</TableCell>
                    <TableCell className="text-[#f58220] font-medium">{l1}</TableCell>
                    <TableCell className="text-[#f58220] font-medium">{l2}</TableCell>
                    <TableCell className="text-red-500 font-medium">{l3}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
function CourseCard({ course, delay, expanded, onToggle }: {
  ...
  const penaltyColor = p.level === "none" ? "#22c55e" : p.level === "L1" ? "#f58220" : "#ef4444";
  const penaltyVariantClass = p.level === "none"
  ...
  return (
    <Card className="bg-[var(--color-bg-card)] border-[var(--color-border)]" style={{ animationDelay: `${delay}s` }}>
      <CardContent className="p-4">
        {/* Header */}
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0 mr-4">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <Badge variant="outline" className={course.courseType === "core" ? "text-green-500 border-green-500/30 bg-green-500/10 text-[10px]" : "text-[#f58220] border-[#f58220]/30 bg-[#f58220]/10 text-[10px]"}>
                {course.courseType}
              </Badge>
            ...
            <Badge variant="outline" className={`${penaltyVariantClass} text-[11px]`}>
        ...
        {/* Penalty detail */}
        <div className="rounded-lg px-3 py-2 text-xs mb-2 bg-[var(--color-bg-secondary)]" style={{ color: penaltyColor }}>
          {p.level === "none"
        ...
        <div className="flex gap-3 text-[11px] mb-1 text-[var(--color-text-muted)]">
          ...
          <span className={p.effectiveAbsences >= p.thresholds.L1 ? "text-[#f58220]" : ""}>L1: {p.thresholds.L1}</span>
          <span className={p.effectiveAbsences >= p.thresholds.L2 ? "text-red-500" : ""}>L2: {p.thresholds.L2}</span>
          <span className={p.effectiveAbsences >= p.thresholds.L3 ? "text-red-500" : ""}>L3: {p.thresholds.L3}</span>
        </div>
        ...
        >
          {expanded
            ...
            : <><ChevronDownIcon className="w-3.5 h-3.5 mr-1" /> View Attendance Log</>
          }
        </Button>
            ...
            {course.log.length === 0
              ...
              : (
                <Table>
                  <TableHeader>
                    <TableRow className="border-[var(--color-border)]">
                      <TableHead className="text-[var(--color-text-muted)] text-xs">Date</TableHead>
                      <TableHead className="text-[var(--color-text-muted)] text-xs">Slot</TableHead>
                      <TableHead className="text-[var(--color-text-muted)] text-xs">Status</TableHead>
                      <TableHead className="text-[var(--color-text-muted)] text-xs">Swipe</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {course.log.map(l => (
                      <TableRow key={l.sessionId} className="border-[var(--color-border)]">
                        <TableCell className="text-[var(--color-text-primary)] text-xs py-2">{l.date}</TableCell>
                        <TableCell className="text-[var(--color-text-secondary)] text-xs py-2">Slot {l.slot}</TableCell>
                        <TableCell className="py-2">
                          <Badge variant="outline" className={`text-[11px] ${
                            l.status === "P"  ? "text-green-500 border-green-500/30 bg-green-500/10" :
                            l.status === "AB" ? "text-red-500 border-red-500/30 bg-red-500/10" :
                            "text-[#f58220] border-[#f58220]/30 bg-[#f58220]/10"
                          }`}>
                            {l.status === "P" ? "Present" : l.status === "AB" ? "Absent" : l.status === "LT" ? "Late" : l.status}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-[var(--color-text-muted)] text-xs py-2">{l.swipeTime || "—"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
